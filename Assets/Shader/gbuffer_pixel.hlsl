...
struct GBUFFER_OUT
{
    ...
    // Metallic (R) + Emission (G) + Optional (B, A)
...
GBUFFER_OUT main(PS_INPUT input)
{
    ...
    // float ao = t_AmbientOcclusion.Sample(s_TextureSampler, input.v_UV).r;
    float3 normalSample = t_NormalMap.Sample(s_TextureSampler, input.v_UV);
    float metallic = t_MetallicMap.Sample(s_TextureSampler, input.v_UV);
    float roughness = t_RoughnessMap.Sample(s_TextureSampler, input.v_UV);
    ...
    output.g_AlbedoAO = float4(albedo * u_Material.BaseColorFactor, 1.0);
    output.g_NormalRough = float4(worldNormal * 0.5 + 0.5, roughness * u_Material.Roughness);
    output.g_MetalEmit = float4(metallic * u_Material.Metallicness, 0.0f, 0.0f, 1.0f);
