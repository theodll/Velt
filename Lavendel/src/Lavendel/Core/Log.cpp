...
namespace Lavendel {
    ...
    void Log::Init()
    {
        ...
        s_FileSink = std::make_shared<spdlog::sinks::basic_file_sink_mt>("Lavendel.log", true);
        s_FileSink->set_pattern("[%T] [%l] %n: %v");

        std::vector<spdlog::sink_ptr> coreSinks = { consoleSink, s_FileSink };
        s_CoreLogger = std::make_shared<spdlog::logger>("LAVENDEL", coreSinks.begin(), coreSinks.end());
