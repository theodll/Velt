...
namespace Velt
{
	...
	DefferedShaderInput::DefferedShaderInput(Ref<Camera> pCamera)
	{
		...
		m_CameraUBOBinding = VT_DEFFERED_SET_BINDING_CAMERA_UBO;
		...
		for (u32 i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
		{
			...
			m_LightUBOs[i] = UniformBuffer::Create(sizeof(LightUBO));
	...
	void DefferedShaderInput::UpdateData()
	{
		...
		*/
		...
		if (m_ActiveScene)
		{
			...
			{
				auto&& [transform, lightComponent] = view.get<TransformComponent, LightComponent>(entity);
