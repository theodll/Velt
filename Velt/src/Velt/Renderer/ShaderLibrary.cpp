...
namespace Velt 
{
	...
	VkShaderModule ShaderLibrary::CreateVulkanModule(const std::vector<u32>& bytes)
	{
		...
		VT_VK_CHECK(
			vkCreateShaderModule(VulkanContext::GetDevice()->device(), &createInfo, VT_NULL_HANDLE, module),
	...
	static RHI::DescriptorType FromSpvReflectDescriptorType(SpvReflectDescriptorType type)
	{
		...
		case SPV_REFLECT_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
			return RHI::DescriptorType::COMBINED_IMAGE_SAMPLER;

		case SPV_REFLECT_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
			return RHI::DescriptorType::SAMPLED_IMAGE;

		case SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_IMAGE:
			return RHI::DescriptorType::STORAGE_IMAGE;

		case SPV_REFLECT_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER:
			return RHI::DescriptorType::UNIFORM_TEXEL_BUFFER;

		case SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
			return RHI::DescriptorType::STORAGE_TEXEL_BUFFER;
		...
		case SPV_REFLECT_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
			return RHI::DescriptorType::INPUT_ATTACHMENT;
	...
	std::vector<DescriptorSetLayoutData> ReflectShader(const std::vector<u32>& bytes)
	{
		...
		u32 count{};
		result = spvReflectEnumerateDescriptorBindings(&module, &count, VT_NULL_HANDLE); 
		...
		// (c) Khronos Group - Spirv Reflect - https://github.com/KhronosGroup/SPIRV-Reflect/blob/main/examples/main_descriptors.cpp
		// slightly modified
		...
		for (size_t i_set = 0; i_set < sets.size(); ++i_set) {
			...
			layout_binding.stage = FromSpvShaderStage(module.shader_stage);
			}
