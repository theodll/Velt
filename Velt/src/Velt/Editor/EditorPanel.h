...
namespace Velt::Editor 
{
	...
	namespace Shared 
	{
		...
		static void DrawVec4Control(const std::string& label, Vector4* values, float resetValue = 0.0f, float columnWidth = 100.0f, float speed = 0.05f)
		{
			...
			FontLibrary::Get().Push(VT_FONT_TYPE_BOLD);
			if (ImGui::Button("W", buttonSize)) values->y = resetValue;
			FontLibrary::Get().Pop();
			ImGui::PopStyleColor(3);

			ImGui::SameLine();
			ImGui::PushItemWidth(itemWidth - buttonSize.x);
			ImGui::DragFloat("##W", &values->w, speed, 0.0f, 0.0f, "%.2f");
			ImGui::PopItemWidth();
			ImGui::SameLine();
